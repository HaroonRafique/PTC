#!/usr/bin/env python3
"""Apply the latest survey error table and compare PyPTC orbit with MAD-X Twiss."""
...
from common import (
    MADX_DIR,
    SURVEY_TO_LATTICE_LATEST_CORRECTED_ERROR_TABLE,
...
def main() -> None:
    ...
    error_table = SURVEY_TO_LATTICE_LATEST_CORRECTED_ERROR_TABLE
    ...
    if len(records) == 38 and len(applied) != 38:
        raise RuntimeError(f"Survey table produced {len(applied)} applications for {len(records)} records")
    ptc.update_twiss()
    edited_rows = ptc.all_node_twiss_orbit()
    orbit = orbit_response_rows(bare_rows, edited_rows)
    plot_orbit_response(out / "full_error_table_pyptc_orbit.png", orbit, labels=("bare", "latest full error table"))
